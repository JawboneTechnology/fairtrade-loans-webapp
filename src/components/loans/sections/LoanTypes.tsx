        ...
        <div className="space-y-3">
          {Array.isArray(loanTypes) && loanTypes.length > 0 ? (
            loanTypes.map((loan) => (
              <label
                key={loan.id}
                className={`flex items-center gap-4 cursor-pointer border-2 rounded-2xl p-4 transition-all duration-200 transform hover:scale-[1.02] ${loanApplication?.loan_type_id === loan.id
              ...
              >
                ...
                />
                <div
                  className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 ${loanApplication?.loan_type_id === loan.id
                      ? "bg-primary"
                ...
                <span className={`font-semibold flex-1 ${loanApplication?.loan_type_id === loan.id
                    ? "text-primary"
                    : "text-dark"
                  }`}>
                  {loan.name}
            ...
            <div className="text-sm text-gray-400">No loan types available.</div>
          )}
        </div>
      </div>
    </>
